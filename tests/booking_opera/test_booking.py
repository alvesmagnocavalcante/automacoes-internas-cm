...
class BookingTests(TestCase):
    ...
    def test_browser_is_created_in_visible_mode_by_default(self):
        class Options:
            def __init__(self):
                self.arguments = []
    ...
    def test_browser_can_run_headless_when_requested(self):
        class Options:
            def __init__(self):
                self.arguments = []
        ...
        options = Options()
        with (
            patch.dict("os.environ", {"BOOKING_HEADLESS": "true"}, clear=True),
            patch.object(booking_browser, "ChromiumOptions", return_value=options),
            patch.object(booking_browser, "Chromium", return_value=object()),
        ):
            booking_browser.create_browser()
